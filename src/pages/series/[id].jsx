...
function Movie({ data }) {
    ...
    return <>
        <NextSeo
            title={`${data?.name} | Series | Movie App`}
        ...
        <div style={{ zIndex: '-1', "--card-banner": 'url(' + backdropImage + ')' }} className="absolute left-0 top-0 hero-image w-full h-[40rem] text-white">
                ...
                <div className="text-black lg:text-white px-6 xl:px-0 max-w-7xl w-full flex flex-col lg:flex-row items-center gap-8 pt-56 lg:pt-0" >
                    ...
                    <div className="text-center lg:text-left">
                        ...
                        <div className="flex justify-center lg:justify-start items-center gap-4">
                            <div className="text-center">{data?.first_air_date?.split('-')[0]}</div>
            ...
            <div className="mt-56 lg:mt-5 flex items-center justify-center w-full">
                <div className="max-w-7xl w-full py-4 pb-24 px-8 xl:px-0 space-y-24">
                    ...
                    <div>
                        <CardGroup seeMore={false} value="seasons" cardTypes='horizontal' gridClass="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-7 gap-6 mt-4" title="Seasons" data={data?.seasons?.filter(a => a.poster_path !== null)} />
                    </div>
                </div>
            </div>
        </div>
...
export async function getServerSideProps(ctx) {
    const _result = await (await fetch(`https://api.themoviedb.org/3/tv/${ctx?.query?.id}?api_key=3044afc915e1301ae1d9551614db3711&append_to_response=videos,credits,reviews`).then(r => r.json()));
